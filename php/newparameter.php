...
<form action="newparameter.php" method="GET">
  ...
  <input type="submit" name='id_param' value=$value->getLibelle()>
</form>
...
if (isset($_GET["corde"]) && isset($_GET["nb_points"])) {
  ...
  $addParameterQuery->execute(array('libelle'=>$_GET["libelle"], 'corde'=>$_GET["corde"], 'tmax_prc'=>$_GET["tmax_prc"], 'tmax_mm'=>$newParameter->getTmax_mm(), 'fmax_prc'=>$_GET["tmax_prc"], 'fmax_mm'=>$newParameter->getFmax_mm(), 'nb_points'=>$_GET["nb_points"]
